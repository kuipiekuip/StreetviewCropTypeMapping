...
def process_road_data(road_data, geo_idx, geo_data):
    ...
    for element in road_data["elements"]:
        if element["type"] == "way":
            ...
            if any(keyword in tags for keyword in keywords):
                try:
                    process_way_element(
                        element, road_points, field_points, original_points
                    )
    ...
    for point in road_points:
        ...
        if distance < min_distance:
            ...
            nearest_road_point_info = (
                x,
                y,
                bearing,
            )  # Store the nearest point with its bearing
    ...
    if nearest_road_point_info:
        ...
        all_road_points.append(
            (geo_idx,)
            + nearest_road_point_info
            + (geo_data["CONDITIE"][geo_idx], geo_data["INSPECTIED"][geo_idx])
        )
...
def process_line_points(line, road_points, field_points):
    ...
    for j, (x, y) in enumerate(line.coords):
        if j > 3 and j < len(line.coords) - 3:
            ...
            p1 = compute_point_on_field(
                to_point, (bearing + 90) % 360, PERPENDICULAR_DISTANCE
            )
            p2 = compute_point_on_field(
                to_point, (bearing + 270) % 360, PERPENDICULAR_DISTANCE
            )
