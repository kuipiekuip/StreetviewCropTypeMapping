...
SHAPEFILE_PATH = '/Users/marco/Library/CloudStorage/OneDrive-DelftUniversityofTechnology/Control&Simulation/deeplearning/assignment project/StreetviewCropTypeMapping/BomenPoints/bomen.shp'
...
def compute_point_on_field(from_point, theta, distance):
    ...
    Args:
        from_point (tuple): A tuple containing the latitude and longitude of the starting point.
        theta (float): The bearing angle in degrees.
        distance (float): The distance from the starting point in meters.

    Returns:
        tuple: A tuple containing the latitude and longitude of the calculated point.
    """
...
def process_geometry(geometry, geo_idx):
    ...
    Args:
        geometry (shapely.geometry): The geometry object to be processed.
        geo_idx (int): The index of the geometry object.
    ...
    if geometry.geom_type == "Point":
        geometry = geometry.simplify(tolerance, preserve_topology=True)
        print(geometry, "this is the geometry bitches")
        lon , lat = geometry.x, geometry.y
        point_query = create_overpass_query(lon , lat)
        road_data = fetch_overpass_data(point_query)
        print(road_data)

    else:
        print()
        print("ITS NOT A POINT WARNING")
        print("WARNING")
        print(geo_idx, "GEO_index")
        print()
        return
